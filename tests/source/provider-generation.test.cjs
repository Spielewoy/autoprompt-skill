...
test('provider-native prompts are deterministic generated views', () => {
  ...
  assert.equal(outputs.size, contract.personas.length * 10 + contract.frameworks.length * 10 + 49)
...
test('every canonical persona has complete forms for all ten public providers', () => {
  const contract = JSON.parse(fs.readFileSync(CONTRACT_PATH, 'utf8'))
