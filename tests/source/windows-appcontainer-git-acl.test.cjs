...
test('complete resource and launcher sources compile with the native Git ACL regression', {timeout:90000},t=>{
  ...
  const script='$ErrorActionPreference="Stop";$files=[string[]]@($env:AP_GIT_ACL_NATIVE,$env:AP_GIT_ACL_RESOURCES,$env:AP_GIT_ACL_FIXTURE);if($env:AP_GIT_ACL_STUB){Add-Type -Path ($files+@($env:AP_GIT_ACL_STUB))}else{Add-Type -Path $files -ReferencedAssemblies @("System.dll","System.Core.dll","System.Security.dll","System.Web.Extensions.dll")};[Console]::Write("compiled")'
  const result=cp.spawnSync(shell(),['-NoLogo','-NoProfile','-NonInteractive','-Command',script],{encoding:'utf8',timeout:60000,windowsHide:true,env:{...process.env,AP_GIT_ACL_NATIVE:sources[0],AP_GIT_ACL_RESOURCES:sources[1],AP_GIT_ACL_FIXTURE:sources[2],AP_GIT_ACL_STUB:process.platform==='win32'?'':stub}})
...
test('native Windows .git grants deny mutation and restore inheritance after owned drain', {skip:process.platform!=='win32',timeout:600000},t=>{
  ...
  for(const key of ['restoredProtection','partialApplyRestored','foreignProtectionRefused','unrelatedAcePreserved','newGitLabelPreserved','packageGrantsRemoved'])assert.equal(proof[key],true)
