...
const LOCAL_BUILD_MARKER = 'codex-v2-release-status: local-v1.0.10-build-not-published'
const SVG_LOCAL_BUILD_MARKER = 'data-release-status="local-v1.0.10-build-not-published"'
...
test('Codex local-record user docs define fail-closed retention and export authority', () => {
  ...
  assert.match(readme, /local v1\.0\.10 build/i)
  assert.match(readme, /not (?:been )?published/i)
  assert.match(readme, /codex-v2-local-records\.md/)
  assert.match(guide, /local Codex v1\.0\.10 build/i)
...
test('Codex target route docs consistently identify the local v1.0.10 build as not published', () => {
  ...
  for (const relative of PUBLIC_ROUTE_DOCS) {
    ...
    if (p12Status !== 'DONE') {
      ...
      assert.ok(source.includes(expected), `${relative}: identifies the local v1.0.10 build as not published`)
