...
const options = { skip: process.platform !== 'win32' ? 'Windows AppContainer loopback proof requires Windows' : false, timeout: 300000 }
...
test('zero-capability AppContainer permits same-container loopback but denies controller loopback', options, async t => {
  ...
  assert.equal(result.workerIdentity, admission.workerIdentity)
