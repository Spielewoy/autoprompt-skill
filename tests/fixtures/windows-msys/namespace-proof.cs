...
public static class MsysNamespaceProof {
 ...
 static void CheckLeafSecurity(IntPtr handle,String profile,String other,Boolean world){var sd=Security(handle);Require(sd.SystemAcl!=null&&sd.SystemAcl.Count==1,"leaf-label-count");var label=sd.SystemAcl[0];Byte[] labelBytes=new Byte[label.BinaryLength];label.GetBinaryForm(labelBytes,0);Require(labelBytes[0]==0x11&&labelBytes[1]==0&&BitConverter.ToUInt32(labelBytes,4)==1&&new SecurityIdentifier(labelBytes,8).Value=="S-1-16-4096","leaf-low-label");Require(sd.DiscretionaryAcl!=null,"leaf-null-effective-DACL");Boolean inherited=false,hasWorld=false;foreach(GenericAce entry in sd.DiscretionaryAcl){var ace=entry as CommonAce;if(ace==null||ace.AceQualifier!=AceQualifier.AccessAllowed)continue;Require(ace.SecurityIdentifier.Value!=other&&ace.SecurityIdentifier.Value!="S-1-15-2-1"&&ace.SecurityIdentifier.Value!="S-1-15-2-2","foreign-profile-leaf-grant");if(ace.SecurityIdentifier.Value==profile&&(ace.AceFlags&AceFlags.Inherited)!=0)inherited=true;if(ace.SecurityIdentifier.Value=="S-1-1-0")hasWorld=true;}Require(inherited,"missing-inherited-profile-leaf-grant");if(world)Require(hasWorld,"WORLD-fixture-did-not-preserve-explicit-ACE");}
 ...
 static void Child(String[] args){
  Boolean allowed=args[1]=="allow";Int32 count=0;var failures=new List<String>();
  for(Int32 i=2;i<args.Length;i++){
   String name=args[i];IntPtr handle=IntPtr.Zero;Int32 status;
   ...
   try{
    Diagnostic("open",(allowed?"same":"other")+" index="+(i-2)+" status="+unchecked((UInt32)status).ToString("X8")+" handle="+(handle!=IntPtr.Zero));
    try{if(allowed){Success(status,"same-profile-open");Require(handle!=IntPtr.Zero,"empty-leaf-handle");if(name.EndsWith("event",StringComparison.Ordinal))Success(NtSetEvent(handle,IntPtr.Zero),"set-event");else MapWitness(handle,false);}else Require(status==unchecked((Int32)0xc0000022)&&handle==IntPtr.Zero,"cross-profile-not-denied:"+unchecked((UInt32)status).ToString("X8"));count++;}
  ...
  Require(failures.Count==0,"leaf-open-failures:"+String.Join(";",failures));Console.Write((allowed?"allowed:":"denied:")+count);
 }
 ...
 static void RunChild(String executable,String profileSid,IntPtr profile,Boolean allowed,String[] names,String cwd){
  ...
  Require(result.ExitCode==0&&errors.Length==0&&lines[lines.Length-1]==(allowed?"allowed:":"denied:")+names.Length,"child-result:"+result.ExitCode+":"+errors);
 }
