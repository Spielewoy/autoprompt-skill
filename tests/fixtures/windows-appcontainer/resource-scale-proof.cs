...
public static class ResourceScaleProof {
  ...
  static void ObserveIdentities(string target){
    IntPtr hint=CreateFileW("\\\\?\\"+Path.GetPathRoot(target),0x80,7,IntPtr.Zero,3,0x02200000,IntPtr.Zero);int hintError=ValidHandle(hint)?0:Marshal.GetLastWin32Error();var records=new List<object>();
    ...
    Console.Error.WriteLine("RESOURCE_SCALE_ID_OBSERVATIONS:"+Json.Serialize(new{schema=1,scope="controller-after-owned-child-drain-no-absence-classification",hintOpened=ValidHandle(hint),hintError=hintError,baseline=BaselineIdentities,records=records.ToArray()}));
  }
