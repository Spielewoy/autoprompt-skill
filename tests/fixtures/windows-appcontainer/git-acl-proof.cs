...
public static class GitAclProof {
  ...
  static readonly string[] DenialNames={"overwrite","append","rename-file","delete-file","rename-git","new-git-file","nested-write","nested-create","protected-write","git-file-WRITE_DAC","git-directory-WRITE_DAC","target-WRITE_DAC","target-DELETE_CHILD","target-DELETE","scratch-DELETE_CHILD","scratch-DELETE","git-DELETE_CHILD"};
  ...
  static int Child(string target,string scratch,string sid){Phase="child-controls";CheckToken(sid);var outcomes=new List<string>();string git=Path.Combine(target,".git"),guard=Path.Combine(git,"guard"),nested=Path.Combine(git,"nested"),protectedFile=Path.Combine(git,"protected","guard");
    ...
    foreach(string root in new[]{target,scratch}){string ownedFile=Path.Combine(root,"new-delete-file"),ownedDirectory=Path.Combine(root,"new-delete-directory");File.WriteAllText(ownedFile,"delete me");Directory.CreateDirectory(ownedDirectory);File.Delete(ownedFile);Directory.Delete(ownedDirectory);Need(!File.Exists(ownedFile)&&!Directory.Exists(ownedDirectory),"new-owned-delete");}
    ...
    DeniedHandle(guard,0x40000,"git-file-WRITE_DAC",outcomes);DeniedHandle(git,0x40000,"git-directory-WRITE_DAC",outcomes);DeniedHandle(target,0x40000,"target-WRITE_DAC",outcomes);DeniedHandle(target,0x40,"target-DELETE_CHILD",outcomes);DeniedHandle(target,0x10000,"target-DELETE",outcomes);DeniedHandle(scratch,0x40,"scratch-DELETE_CHILD",outcomes);DeniedHandle(scratch,0x10000,"scratch-DELETE",outcomes);DeniedHandle(git,0x40,"git-DELETE_CHILD",outcomes);
    Need(outcomes.SequenceEqual(DenialNames.Select(x=>x+":5")),"closed-child-outcomes");Console.Write(new JavaScriptSerializer().Serialize(new{appContainer=true,architecture=NativeArchitecture(),denials=outcomes,positiveOperations=12}));return 0;
  }
  ...
  static void RootGrantShape(string path,string sid){var aces=Security(path).DiscretionaryAcl.Cast<GenericAce>().OfType<CommonAce>().Where(ace=>ace.SecurityIdentifier.Value==sid).ToArray();Need(aces.Length==2,"root-package-ACE-count");Need(aces.Any(ace=>ace.AceFlags==AceFlags.None&&ace.AccessMask==(0x1201bf&~0x40)),"root-self-package-ACE");Need(aces.Any(ace=>ace.AceFlags==(AceFlags.ObjectInherit|AceFlags.ContainerInherit|AceFlags.InheritOnly)&&ace.AccessMask==(0x1301bf&~0x40)),"root-child-package-ACE");}
  ...
  static int Controller(string target,string scratch,string executable,string control){
    ...
    try{
      ...
      var applied=Paths(git).ToDictionary(x=>x,Security);RootGrantShape(target,sid);RootGrantShape(scratch,sid);foreach(var sd in applied.Values){Need(Protected(sd),"git-must-be-protected");var packageAces=sd.DiscretionaryAcl.Cast<GenericAce>().OfType<CommonAce>().Where(x=>x.SecurityIdentifier.Value==sid).ToArray();Need(packageAces.Length==1&&packageAces[0].AceQualifier==AceQualifier.AccessAllowed&&packageAces[0].AccessMask==0x1200a9&&(packageAces[0].AceFlags&AceFlags.Inherited)==0,"git-read-only-package-ACE");}
