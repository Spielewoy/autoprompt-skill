...
public static class GitAclProof {
  ...
  static int Controller(string target,string scratch,string executable,string control){
    ...
    try{
      ...
      foreach(var item in originals){var after=Security(item.Key);Need(Protected(after)==Protected(item.Value)&&after.Owner.Value==item.Value.Owner.Value&&Label(after)==Label(item.Value),"original-metadata-restored");Need(Aces(after,sid).SequenceEqual(item.Key==concurrent?concurrentAces:Aces(item.Value,sid)),"unrelated-ACEs-preserved");}
