...
public static class GitAclProof {
  ...
  static void InvalidPlans(string json){
    Refuses("WINDOWS_RESOURCE_INVALID",()=>WindowsAppContainerResourcesNative.ReadPlan(json.Replace("\"schemaVersion\":2","\"schemaVersion\":1")));
  ...
  static int Controller(string target,string scratch,string executable,string control){
    ...
    try{
      ...
      string concurrent=Path.Combine(git,"guard");AddUnrelated(concurrent);string[] concurrentAces=Aces(Security(concurrent),sid);string newGit=Path.Combine(git,"controller-new");File.WriteAllText(newGit,"controller new");var newGitBefore=Security(newGit);
      ...
      foreach(var item in originals){var after=Security(item.Key);Need(Protected(after)==Protected(item.Value)&&after.Owner.Value==item.Value.Owner.Value&&Label(after)==Label(item.Value),"original-metadata-restored");bool same=Aces(after,sid).SequenceEqual(item.Key==concurrent?concurrentAces:Aces(item.Value,sid));if(!same)ObserveRestoreMismatch(item.Key,target,scratch,control,sid,item.Value,appliedOriginals[item.Key],beforeRestore[item.Key],item.Key==concurrent?beforeRestore[item.Key]:item.Value,after);Need(same,"unrelated-ACEs-preserved");}
