...
async function roundtrip(child, value, expectedStdout, expectedStderr) {
  let stdout = '', stderr = ''
...
if (mode === 'ipc-child') {
  ...
  nestedChild(process.argv[3], process.argv[4] === '1').catch(error => { console.error(error); process.exitCode = 1 })
} else {
