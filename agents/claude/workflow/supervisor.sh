...
#   supervisor.sh --agents "auto:strong,cheap" --model-registry ~/.claude/autoprompt-models.json --cmd "claude -p" "<mission>"
#
...
#                          CLAUDE_CODE_SUBAGENT_MODEL from the CHILD ONLY (env -u) so the
#                          skill's `model: inherit` contract resolves to the real session/
#                          --model selection instead of the operator's force-pin. Set to 1
#                          to preserve the operator's pin and let it reach the child as before.
...
CASTING_API_KEY_ENV=""
CASTING_AUTH_TOKEN=""
CAPABILITY_PROVIDER="claude-code"
...
resolve_model_casting() {
  ...
  [ -z "$control" ] || [ "$control" = "off" ] || {
    ...
    CASTING_OPUS=$(casting_json_field aliases.opus) || return 2
    CASTING_SONNET=$(casting_json_field aliases.sonnet) || return 2
    CASTING_HAIKU=$(casting_json_field aliases.haiku) || return 2
    CASTING_BASE_URL=$(casting_json_field endpoint.baseUrl) || return 2
    CASTING_API_KEY_ENV=$(casting_json_field endpoint.apiKeyEnv) || return 2

    if [ "${AUTOPROMPT_KEEP_MODEL_PIN:-0}" = "1" ] && [ -n "${CLAUDE_CODE_SUBAGENT_MODEL:-}" ]; then
      echo "supervisor: AUTOPROMPT_KEEP_MODEL_PIN=1 conflicts with model casting because CLAUDE_CODE_SUBAGENT_MODEL is set" >&2
      return 2
    fi
    if [ -n "$CASTING_API_KEY_ENV" ]; then
      CASTING_AUTH_TOKEN=$(printenv "$CASTING_API_KEY_ENV" 2>/dev/null || true)
      [ -n "$CASTING_AUTH_TOKEN" ] || {
        echo "supervisor: model casting credential environment variable is unset or empty: $CASTING_API_KEY_ENV" >&2
        return 2
      }
    fi
  }
...
run_launcher() {
  ...
  if [ "$DRY_RUN" -eq 1 ]; then
    ...
    # Neutralize the operator's CLAUDE_CODE_SUBAGENT_MODEL for the CHILD CLI ONLY.
    # That var has HIGHEST precedence in the harness and force-pins every (even
    # nested) subagent onto a fixed model, defeating the skill's `model: inherit`
    # contract. A child cannot mutate its parent's env, so the LAUNCH layer is the
    # only correct place to strip it: `env -u` removes it from the child's
    # environment WITHOUT touching this supervisor's own env or the operator's
    # shell/profile/CLAUDE.md/settings.json. `inherit` then resolves to the real
    # session/--model selection. Idempotent: env -u of an unset/inherit var is a
    # no-op, never an error. AUTOPROMPT_KEEP_MODEL_PIN=1 opts OUT (keeps the pin).
    (
      ...
      if [ "${AUTOPROMPT_KEEP_MODEL_PIN:-0}" != "1" ]; then
      ...
      if [ "$CASTING_ENABLED" = "true" ]; then
        ...
        export ANTHROPIC_DEFAULT_OPUS_MODEL="$CASTING_OPUS"
        export ANTHROPIC_DEFAULT_SONNET_MODEL="$CASTING_SONNET"
        export ANTHROPIC_DEFAULT_HAIKU_MODEL="$CASTING_HAIKU"
        if [ -n "$CASTING_BASE_URL" ]; then export ANTHROPIC_BASE_URL="$CASTING_BASE_URL"; fi
        if [ -n "$CASTING_API_KEY_ENV" ]; then export ANTHROPIC_AUTH_TOKEN="$CASTING_AUTH_TOKEN"; fi
      fi
