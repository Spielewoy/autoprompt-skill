...
.EXAMPLE
  powershell -File supervisor.ps1 --agents "auto:strong,cheap" --model-registry "$HOME\.claude\autoprompt-models.json" --cmd "claude -p" "<mission>"
...
.NOTES
  Env (all optional), mirroring supervisor.sh:
    ...
    AUTOPROMPT_MODEL_REGISTRY  optional registry path; defaults to ~/.claude/autoprompt-models.json for auto modes
    ...
    AUTOPROMPT_KEEP_MODEL_PIN  when "1", SKIP neutralizing CLAUDE_CODE_SUBAGENT_MODEL
                          for the child CLI. By DEFAULT (unset) the supervisor strips
                          CLAUDE_CODE_SUBAGENT_MODEL from the CHILD ONLY so the skill's
                          `model: inherit` contract resolves to the real session/--model
                          selection instead of the operator's force-pin. Set to 1 to
                          preserve the operator's pin and let it reach the child as before.
...
function Invoke-Supervisor {
    ...
    $castingApiKeyEnv = ''
    $castingAuthToken = ''
    $capabilityProvider = 'claude-code'
    ...
    if (-not [string]::IsNullOrEmpty($selectorControl) -and $selectorControl -ne 'off') {
        ...
        if ($castingEnabled) {
            $keepModelPin = (Get-EnvOrDefault 'AUTOPROMPT_KEEP_MODEL_PIN' '0') -eq '1'
            if ($keepModelPin -and -not [string]::IsNullOrEmpty($env:CLAUDE_CODE_SUBAGENT_MODEL)) {
                [Console]::Error.WriteLine('supervisor: AUTOPROMPT_KEEP_MODEL_PIN=1 conflicts with model casting because CLAUDE_CODE_SUBAGENT_MODEL is set')
                return 2
            }
            $baseUrlProperty = $casting.endpoint.PSObject.Properties['baseUrl']
            if ($null -ne $baseUrlProperty) { $castingBaseUrl = [string]$baseUrlProperty.Value }
            $apiKeyProperty = $casting.endpoint.PSObject.Properties['apiKeyEnv']
            if ($null -ne $apiKeyProperty) { $castingApiKeyEnv = [string]$apiKeyProperty.Value }
            if (-not [string]::IsNullOrEmpty($castingApiKeyEnv)) {
                $castingAuthToken = [Environment]::GetEnvironmentVariable($castingApiKeyEnv)
                if ([string]::IsNullOrEmpty($castingAuthToken)) {
                    [Console]::Error.WriteLine("supervisor: model casting credential environment variable is unset or empty: $castingApiKeyEnv")
                    return 2
                }
            }
        }
    ...
    function Invoke-Launcher {
        ...
        # Neutralize the operator's CLAUDE_CODE_SUBAGENT_MODEL for the CHILD CLI
        # ONLY. That var has HIGHEST precedence in the harness and force-pins every
        # (even nested) subagent onto a fixed model, defeating the skill's
        # `model: inherit` contract. PowerShell env is process-scoped, so the
        # faithful analog of the .sh `env -u` is save -> set $null -> restore in
        # `finally`: the backgrounded child inherits the var UNSET while this
        # supervisor process's own value (and the operator's shell/profile) is
        # restored intact after the child exits. `inherit` then resolves to the real
        # session/--model selection. Idempotent: a null/inherit/unset prior value
        # round-trips with no error. AUTOPROMPT_KEEP_MODEL_PIN=1 opts OUT.
        $keepModelPin = (Get-EnvOrDefault 'AUTOPROMPT_KEEP_MODEL_PIN' '0') -eq '1'
        $previousSubagentModel = $env:CLAUDE_CODE_SUBAGENT_MODEL
        ...
        $previousOpusModel = $env:ANTHROPIC_DEFAULT_OPUS_MODEL
        $previousSonnetModel = $env:ANTHROPIC_DEFAULT_SONNET_MODEL
        $previousHaikuModel = $env:ANTHROPIC_DEFAULT_HAIKU_MODEL
        ...
        if (-not $keepModelPin) { $env:CLAUDE_CODE_SUBAGENT_MODEL = $null }
        ...
        if ($castingEnabled) {
            ...
            $env:ANTHROPIC_DEFAULT_OPUS_MODEL = [string]$casting.aliases.opus
            $env:ANTHROPIC_DEFAULT_SONNET_MODEL = [string]$casting.aliases.sonnet
            $env:ANTHROPIC_DEFAULT_HAIKU_MODEL = [string]$casting.aliases.haiku
            if (-not [string]::IsNullOrEmpty($castingBaseUrl)) {
                $env:ANTHROPIC_BASE_URL = $castingBaseUrl
            }
            if (-not [string]::IsNullOrEmpty($castingApiKeyEnv)) {
                $env:ANTHROPIC_AUTH_TOKEN = $castingAuthToken
            }
        }
        try {
            ...
            $env:ANTHROPIC_DEFAULT_OPUS_MODEL = $previousOpusModel
            $env:ANTHROPIC_DEFAULT_SONNET_MODEL = $previousSonnetModel
            $env:ANTHROPIC_DEFAULT_HAIKU_MODEL = $previousHaikuModel
            $env:ANTHROPIC_BASE_URL = $previousBaseUrl
            $env:ANTHROPIC_AUTH_TOKEN = $previousAuthToken
            $env:CLAUDE_CODE_SUBAGENT_MODEL = $previousSubagentModel
            $restoredModelPin = $env:CLAUDE_CODE_SUBAGENT_MODEL
            if ([string]::IsNullOrEmpty($restoredModelPin)) { $restoredModelPin = 'UNSET' }
            Write-SupLog "model-pin restored to $restoredModelPin"
