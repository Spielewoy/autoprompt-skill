...
`SKILL.md` is authoritative. This file defines the concise roadmap-first gate contract for OpenCode.
...
## 5. Resume, OpenCode configuration, and authority
...
OpenCode agent selection is `inherited-only`: the installed `ap-*` markdown subagents carry no `model` key and inherit the runtime's configured model. It does not use Claude Code model aliases, alias routing, or custom-agent TOML casting. Model and effort never change gates or concurrency.

Record effort as exactly `inherited-only`; omit any effort field and never claim a requested or maximum effort was applied.

The activation profile pins `subagent_depth = 4` and limits task launches to the `ap-*` cast for the chosen hierarchy and concurrency. These are ceilings, never spawn targets.
