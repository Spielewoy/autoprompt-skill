...
- **Agent selection:** `off`/inherit. OpenCode subagents inherit the runtime model, so effort capability is exactly `inherited-only`; never claim a selectable effort.
...
## 8. OpenCode model and effort

OpenCode `ap-*` subagents are native markdown agent definitions with no `model` key: every role inherits the runtime's configured model. OpenCode casting is therefore `inherited-only`; agent selection changes nothing about gates or concurrency, and an explicit model list is not routable through the markdown definitions. Record effort as exactly `inherited-only` and never claim a requested or maximum effort was applied.

Each agent's `permission` block is the recursion brake: the L1 coordinators, `ap-manager`, diagnostic/legacy `ap-preflight-probe` and `ap-intake`, and L3 `ap-implementer` may launch only registered `ap-*` tasks. `ap-implementer` may launch registered `ap-*` L4 leaves for genuinely disjoint verification; all other L3 executors and every L4 leaf deny `task`. Every persona denies `skill`, and the activation profile limits the primary's task launches to `ap-*` as well.
...
```text
OPENCODE_CONFIG=~/.config/opencode/autoprompt.opencode.json opencode
/autoprompt <mission>
```

The activation-only profile merges over the user's ordinary `opencode.json` (which is never edited) and pins `subagent_depth=4`, `share=disabled`, and primary task launches limited to the `ap-*` cast. It requires OpenCode >= 1.18.7. Without `OPENCODE_CONFIG` the installed subagents stay inert: their own permission blocks deny cross-agent launches outside the cast.
