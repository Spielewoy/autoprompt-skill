...
### OpenCode agent selection and effort

OpenCode casting is `inherited-only`: the installed `ap-*` markdown subagents
carry no `model` key and inherit the runtime's configured model. It does **not**
use Claude Code model-family aliases, alias routing, custom-agent TOMLs, or
environment-variable inheritance claims.

- `agents=off` or omitted: the only routable mode; every role inherits the
  runtime model.
- `agents=auto`, `agents=<comma-list>`, `agents=auto:<comma-list>`: not routable
  through native markdown definitions; record `inherited-only` and never pretend
  a selection applied.

Record effort as exactly `inherited-only`: omit any effort field and never claim
a requested or maximum effort was applied.

The recursion brakes are native: each agent's `permission` block denies every
tool the persona lacks. L1 coordinators, `ap-manager`, diagnostic/legacy
`ap-preflight-probe` and `ap-intake`, and L3 `ap-implementer` may launch only
registered `ap-*` tasks; all other L3 executors and every L4 leaf deny `task`.
Every persona denies `skill`, and the activation profile pins
`subagent_depth = 4` with the primary's task launches limited to `ap-*`.
