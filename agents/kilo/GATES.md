...
`SKILL.md` is authoritative. This file defines the concise roadmap-first gate contract for Kilo.
...
## 5. Resume, Kilo configuration, and authority
...
Kilo agent selection is `inherited-only`: the installed `ap-*` markdown subagents carry no `model` key and inherit the runtime's configured model. It does not use Claude Code model aliases, alias routing, or custom-agent TOML casting. Model and effort never change gates or concurrency.
