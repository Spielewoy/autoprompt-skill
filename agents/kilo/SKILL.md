...
```text
KILO_CONFIG=~/.config/kilo/autoprompt.kilo.json kilo
/autoprompt <mission>
```

The activation-only profile merges over the user's ordinary `kilo.json` (which is never edited) and pins `subagent_depth=4`, `share=disabled`, and primary task launches limited to the `ap-*` cast. It requires Kilo >= 7.4.22. Without `KILO_CONFIG` the installed subagents stay inert: their own permission blocks deny cross-agent launches outside the cast.
