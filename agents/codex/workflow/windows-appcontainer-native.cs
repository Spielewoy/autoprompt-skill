...
public static class WindowsAppContainerNative {
 ...
 const UInt32 EXTENDED_STARTUPINFO_PRESENT=0x00080000,CREATE_SUSPENDED=0x4,CREATE_UNICODE_ENVIRONMENT=0x400,CREATE_NO_WINDOW=0x08000000,PROC_THREAD_ATTRIBUTE_SECURITY_CAPABILITIES=0x00020009,PROC_THREAD_ATTRIBUTE_HANDLE_LIST=0x00020002,PROC_THREAD_ATTRIBUTE_MITIGATION_POLICY=0x00020007,JOB_OBJECT_LIMIT_KILL_ON_JOB_CLOSE=0x2000,TOKEN_QUERY=0x8,STARTF_USESTDHANDLES=0x100,GENERIC_READ=0x80000000,GENERIC_WRITE=0x40000000,FILE_SHARE_READ=1,FILE_SHARE_WRITE=2,OPEN_EXISTING=3,FILE_ATTRIBUTE_NORMAL=0x80,HANDLE_FLAG_INHERIT=1,ERROR_BROKEN_PIPE=109;
 // AppContainer enables high-entropy ASLR even when the MSYS PE image does
 // not support it. Its fixed fork heap needs ordinary ASLR; MSYS reapplies
 // this compatibility setting to its own children because it is not inherited.
 const UInt64 MsysCompatibilityMitigation=2UL<<20;
 [StructLayout(LayoutKind.Sequential)] struct MSYS_ASLR_POLICY { public UInt32 Flags; }
 [DllImport("kernel32.dll",EntryPoint="GetProcessMitigationPolicy",SetLastError=true)] static extern Boolean QueryMsysAslr(IntPtr process,UInt32 policy,out MSYS_ASLR_POLICY information,UIntPtr length);
 static void VerifyMsysAslr(IntPtr process){MSYS_ASLR_POLICY policy;Check(QueryMsysAslr(process,1,out policy,(UIntPtr)Marshal.SizeOf(typeof(MSYS_ASLR_POLICY))),"GetProcessMitigationPolicy-msys");if((policy.Flags&4)!=0)throw new InvalidOperationException("WINDOWS_MSYS_ASLR_INCOMPATIBLE");}
 ...
 public static LaunchResult Launch(String executable,String executableSha256,String[] arguments,String cwd,String[] environmentEntries,Int32 timeoutMs,Int32 outputLimit,IntPtr appSid,String expectedSid,String cancellationPath,MsysNamespaceRequest msysRuntime) {
  ...
  IntPtr size=IntPtr.Zero,list=IntPtr.Zero,mitigation=IntPtr.Zero,caps=IntPtr.Zero,job=IntPtr.Zero,limit=IntPtr.Zero,environment=IntPtr.Zero,stdoutRead=IntPtr.Zero,stdoutWrite=IntPtr.Zero,stderrRead=IntPtr.Zero,stderrWrite=IntPtr.Zero,nulRead=IntPtr.Zero,privateNull=IntPtr.Zero,handleList=IntPtr.Zero;PROCESS_INFORMATION pi=new PROCESS_INFORMATION();Boolean assigned=false,confirmedDrain=true;ImageBinding imageBinding=null;MsysNamespaceLease namespaceLease=null;
  try {
   int attributeCount=msysRuntime==null?2:3;InitializeProcThreadAttributeList(IntPtr.Zero,attributeCount,0,ref size);if(size==IntPtr.Zero)throw new InvalidOperationException("attribute-list size");list=Marshal.AllocHGlobal(size);Check(InitializeProcThreadAttributeList(list,attributeCount,0,ref size),"InitializeProcThreadAttributeList");
   ...
   if(mitigation!=IntPtr.Zero)Marshal.FreeHGlobal(mitigation);if(caps!=IntPtr.Zero)Marshal.FreeHGlobal(caps);if(list!=IntPtr.Zero){DeleteProcThreadAttributeList(list);Marshal.FreeHGlobal(list);}if(imageBinding!=null)imageBinding.Dispose();foreach(IntPtr handle in new[]{stdoutRead,stdoutWrite,stderrRead,stderrWrite,nulRead,privateNull})if(handle!=IntPtr.Zero)CloseHandle(handle);if(handleList!=IntPtr.Zero)Marshal.FreeHGlobal(handleList);
  }
 }
}
