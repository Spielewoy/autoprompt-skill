...
public static class WindowsAppContainerNative {
 ...
 static MemberSummary VerifyMembers(IntPtr job,String expectedSid,ImageBinding imageBinding){Int32 capacity=16;for(;;){IntPtr pointer=Marshal.AllocHGlobal(8+capacity*IntPtr.Size);try{UInt32 returned;if(QueryInformationJobObject(job,JobObjectBasicProcessIdList,pointer,(UInt32)(8+capacity*IntPtr.Size),out returned)){Int32 count=Marshal.ReadInt32(pointer,4);LastObservedMembers=(UInt32)count;if(count<0||count>capacity){Mark("job-member-count-result");throw new InvalidOperationException("APP_PROBE_JOB_MEMBER_COUNT_INVALID");}var summary=new MemberSummary{Total=count};for(Int32 i=0;i<count;i++){IntPtr process=OpenProcessForToken((Int32)Marshal.ReadIntPtr(pointer,8+i*IntPtr.Size).ToInt64());if(process==IntPtr.Zero)continue;try{try{if(VerifyAppContainer(process,expectedSid,job,imageBinding,false))summary.SameNode++;else summary.Other++;}catch{if(WaitForSingleObject(process,0)!=0)throw;}}finally{CloseHandle(process);}}LastSameNodeMembers=summary.SameNode;LastOtherImageMembers=summary.Other;return summary;}if(Marshal.GetLastWin32Error()!=234||capacity>=1024)throw new Win32Exception(Marshal.GetLastWin32Error(),"QueryInformationJobObject-members");capacity*=2;}finally{Marshal.FreeHGlobal(pointer);}}}
