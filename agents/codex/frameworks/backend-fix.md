...
- **L2 manager:** builds the 5-field assignment, spawns the L3/L4 worker per required check.
...
## THE END-TO-END DEBUGGING WORKFLOW (brief each phase into the required check it belongs to)

### Phase 0 - REQUIRED CHECK-ZERO: prove the real test suite runs (before any fix)
...
Brief the implementer, and require the final verifier to check the recorded cause, to:
...
Only after wrong-layer evidence, repeated failure, or cross-module uncertainty, dispatch the
...
unpatched code). The required check is default-FAIL: PASS only when the frozen fix LAYER
...
coverage to the original request's bar (default 100% of the feature's surface) - ≥95% of
changed lines is a floor, not the target.

### Phase 5 - INDEPENDENT FINAL VERIFY
Dispatch the one independent final verifier to run, on the REAL repo (returns reproWasRed /
...
record findings with substantive evidence. A blocking finding re-enters as owned work;
an advisory or P1 non-defect decision carries the exact authority receipt without a
severity downgrade. The final verifier confirms every original ask on opened evidence.
...
- **S4 - root cause is outside the owned file/scope** → OUT-OF-SCOPE; climb a tier
  under the canonical route-change rules. Never patch across subsystems inline.
