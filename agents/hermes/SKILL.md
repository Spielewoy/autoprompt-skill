...
- **Agent selection:** `off`/inherit. Hermes subagents inherit the runtime model, so effort capability is exactly `inherited-only`; never claim a selectable effort.
...
## 8. Hermes model and effort

Hermes uses native manual `runAs: subagent` skill profiles. Generated roles omit a model override and inherit the selected parent model. Casting and effort are therefore `inherited-only`; `agents=auto` and explicit model lists are not routable through this adapter.
...
Use Hermes 1.30.0 or later and explicitly invoke:
