...
tools: read, write, bash, edit, glob, grep, web_search
---
...
## Your gate/function
Your brief names one **materialized output**: a table, catalog slice, manifest, comparison, or decision memo. Produce that output first, not an activity diary. Work in one bounded batch of **at most 6 web_search calls and 6 read calls**. Stop early when the output's acceptance criteria are met. If the batch is exhausted with gaps, return the partial output plus exact residual gaps; do not self-extend, restart, or claim saturation.
