...
## 0. Native host mapping (omp / oh-my-pi)

This package targets omp. The orchestration maps onto omp natives as follows:

- **Entry:** the user runs the installed `/autoprompt` command file (or `/skill:autoprompt`). The skill is `disable-model-invocation: true`, so an ordinary task never loads it automatically.
- **Dispatch:** the omp `task` tool. Every task item MUST bind the intended persona's **registered name** as `agent` (e.g. `"agent": "ap-scope-coordinator"`). An anonymous (omitted), `task`, or invented name is an invalid dispatch; a child that spawns must name another registered `ap-*` persona, and the persona `spawns` allowlist enforces this structurally.
- **Workers:** the 25 `ap-*` personas installed under the omp agents root (`skills/autoprompt/agents/` is the payload source; the installer registers them in the platform's native agents directory). Each persona file declares its own `tools`/`spawns`/`model`/`output` frontmatter.
- **Concurrency:** `mode=tokensaver` ⇒ dispatch in waves of at most 6 live workers; `mode=wide` ⇒ every ready disjoint lane in one spawn-all-then-collect wave up to the host ceiling (`task.maxConcurrency`, default 32, the harness-specific agent cap); `mode=custom max_subs=N` ⇒ wave size N. The live ceiling is enforced by wave-batching the parallel task dispatch - OMP's native shape - never by serializing dependent work.
- **Depth:** the hierarchy L0→L1→L2→L3→L4 requires `task.maxRecursionDepth` >= 4. The installer sets it; the frontier check verifies it and fails closed when it is below 4 (depth-denied spawns surface as dispatch failures, never silent degradation).
- **Model routing:** omp workers inherit the active parent model by default (`agents=off`). Explicit routing (`agents=auto`/`agents=<models>`) is supported: resolve selectors via `task.agentModelOverrides`/`modelRoles` and write them as run-scoped project settings (`<governance-root>/.omp/config.yml`) before the first dispatch; task preflight re-reads project settings at every dispatch, so the mapping applies without touching the user's global config. When routing is off, never add a per-agent selector. Unattended supervisors pass the resolved casting to the child session via `AUTOPROMPT_CASTING`.
- **Questions:** use omp's `ask` tool for user-owned choices. **Ledger files:** omp gives the session full filesystem and `bash`/`eval` access, so `PROMPTS.txt`, `ROADMAP.md`, and `GATELOG.md` live at the governance root as plain files; resume re-reads the `GATELOG.md` frontier and the session can be resumed with `omp --resume`.
- **Supervisor:** unattended runs use the shipped `workflow/supervisor.sh` / `supervisor.ps1` driving `omp -p` (headless print mode) with `--auto-approve`.
...
In an attended session, ask all undefined knobs in one question before any repository work. In an unattended supervisor run, do not ask: default to `tokensaver` and `agents=off` and record the assumptions. A permission-bypass flag is not unattendedness.
...
omp supports explicit per-agent model routing: persona frontmatter `model` lists, `task.agentModelOverrides[agentName]` entries in `config.yml`, and `modelRoles.<role>` aliases (`@review`, `@fast`, ...). Resolution precedence at dispatch: `task.agentModelOverrides[agent]` → agent frontmatter `model` → the parent's active model. When `agents=off`, omit every per-agent selector so workers inherit the active model. When `agents=auto` or a model list is selected, resolve each persona's selector through `modelRoles`/overrides before dispatch and record the resolved mapping in the ledger.
...
```text
/autoprompt mode=tokensaver fix the registration race and add a regression test
/autoprompt mode=wide agents=auto build the booking flow from API to checkout
/autoprompt mode=custom max_subs=4 migrate every model
```

Unattended:

```powershell
powershell -File agents/omp/workflow/supervisor.ps1 --agents auto --cmd "omp -p --auto-approve" "<mission>"
```

```sh
sh agents/omp/workflow/supervisor.sh --agents auto --cmd "omp -p --auto-approve" "<mission>"
```
