...
`ask` call before repository or other tool work. Ask concurrency as
...
For omp, enabled casting is a pre-launch supervisor responsibility. omp
supports explicit per-agent model routing through persona frontmatter `model`
lists, `task.agentModelOverrides[agent]`, and `modelRoles.<role>` aliases;
with casting off, workers inherit the parent's active model. When casting is
on, resolve every persona selector through `task.agentModelOverrides`/`modelRoles`
before dispatch and record the resolved mapping in `GATELOG.md`; an unresolved
or stale selector fails closed instead of silently inheriting.
