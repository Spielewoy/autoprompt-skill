...
static struct {
 ...
 int duplicate_error,duplicate_extra,token_closed,direct_opens;DWORD direct_access;
...
static int CloseHandle(HANDLE handle){uintptr_t n=(uintptr_t)handle;if(n==0x900){mock.token_closed++;return 1;}assert(n<512&&mock.handles[n].active);mock.handles[n].active=0;mock.closed++;return 1;}
...
static int DuplicateHandle(HANDLE from,HANDLE source,HANDLE to,HANDLE*result,DWORD access,int inherit,DWORD flags){
 ...
 if(mock.duplicate_error||n>=512||!mock.handles[n].active){mock.error=5;return 0;}
 ...
 if(mock.duplicate_extra)mock.handles[next].access|=0x1000000;
...
static DWORD GetFileType(HANDLE handle){uintptr_t n=(uintptr_t)handle;assert(n!=0x100&&n<512&&mock.handles[n].active);mock.type_queries++;return(DWORD)mock.handles[n].type;}
...
static void* uv__malloc(size_t size){mock.allocation_calls++;if(mock.fail_allocation==mock.allocation_calls)return NULL;void*p=malloc(size);assert(p);mock.allocations++;return p;}
static void uv__free(void*pointer){if(pointer){assert(mock.allocations>0);mock.allocations--;free(pointer);}}
static WCHAR* GetEnvironmentStringsW(void){WCHAR*p=malloc(mock.inherited_length*sizeof(WCHAR));assert(p);memcpy(p,mock.inherited,mock.inherited_length*sizeof(WCHAR));return p;}
static int FreeEnvironmentStringsW(WCHAR*p){free(p);mock.env_frees++;return 1;}

static HANDLE CreateFileW(const WCHAR*name,DWORD access,DWORD share,SECURITY_ATTRIBUTES*sa,DWORD disposition,DWORD flags,HANDLE template){assert(wide_equal(name,L"NUL")&&share==3&&disposition==3&&flags==0&&template==NULL);assert(sa&&sa->nLength==sizeof(*sa)&&sa->lpSecurityDescriptor==NULL&&sa->bInheritHandle==TRUE);mock.direct_opens++;mock.direct_access=access;return(HANDLE)(uintptr_t)0x800;}
