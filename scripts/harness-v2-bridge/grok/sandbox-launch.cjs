...
function createSandboxLaunch(options = {}) {
  const required = ['root', 'sessionHome', 'grokExecutable', 'nodeExecutable', 'model', 'proxyToken', 'relayToken', 'cwd', 'toolPolicyPath', 'toolPolicySha256']
  ...
  const bridge = path.resolve(__dirname), insideBridge = '/opt/autoprompt-grok', insideRoot = '/run/autoprompt', insideSessionHome = '/autoprompt/session', insideGrok = runtime.executable, insidePolicy = `${insideRoot}/tool-policy.json`
  const env = {
    ...
    AUTOPROMPT_GROK_PROXY_TOKEN: options.proxyToken, AUTOPROMPT_GROK_RELAY_TOKEN: options.relayToken, AUTOPROMPT_GROK_RELAY_FD: '0',
  ...
  argv.push('--proc', '/proc', '--dev', '/dev', '--tmpfs', '/tmp', '--bind', options.root, insideRoot, '--bind', options.sessionHome, insideSessionHome, '--ro-bind', options.toolPolicyPath, insidePolicy, '--ro-bind', bridge, insideBridge)
  ...
  return Object.freeze({ executable: '/usr/bin/bwrap', argv, env: {}, cwd: '/', shell: false, stdin: options.relayStdin, insideRoot, insideSessionHome, insideBridge, insidePolicy })
}
