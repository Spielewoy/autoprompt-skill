...
test('actual fixed ACL helper compiles and rejects malformed authority without Win32 calls',()=>{
 ...
 assert.ifError(result.error);assert.equal(result.status,0,result.stdout+result.stderr);assert.equal(result.stderr,'');assert.match(result.stdout,/^actual-compiled-acl-parser-contracts:41\r?\n$/)
})
