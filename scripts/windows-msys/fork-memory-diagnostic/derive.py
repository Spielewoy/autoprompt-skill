...
DEBUG_SUPPORT = r'''
 ...
 [StructLayout(LayoutKind.Sequential)] struct ASLR_POLICY { public UInt32 Flags; }
 ...
 [DllImport("kernel32.dll",SetLastError=true)] static extern Boolean GetProcessMitigationPolicy(IntPtr process,UInt32 policy,out ASLR_POLICY information,UIntPtr length);
 sealed class ForkMemoryTrace { public readonly StringBuilder Text=new StringBuilder();public readonly StringBuilder Policies=new StringBuilder();public readonly HashSet<UInt32> InitialBreakpoints=new HashSet<UInt32>();public readonly HashSet<UInt32> Active=new HashSet<UInt32>();public readonly Dictionary<UInt32,IntPtr> ProcessHandles=new Dictionary<UInt32,IntPtr>();public readonly HashSet<String> Seen=new HashSet<String>();public Int32 Events,Processes,Loads,Spans,QueryError;public Boolean Overflow,Started; }
 static String Hex(UInt64 value){return "0x"+value.ToString("x");}
 static void RecordPolicy(ForkMemoryTrace trace,UInt32 pid,IntPtr process){if(trace.Processes>2048){trace.Overflow=true;return;}ASLR_POLICY policy;Boolean ok=GetProcessMitigationPolicy(process,1,out policy,(UIntPtr)Marshal.SizeOf(typeof(ASLR_POLICY)));Int32 error=ok?0:Marshal.GetLastWin32Error();trace.Policies.Append("policy pid=").Append(pid).Append(" flags=").Append(ok?Hex(policy.Flags):"none").Append(" error=").Append(error).Append('\n');}
 ...
 static Boolean PumpDebug(ForkMemoryTrace trace,UInt32 wait){
  ...
  try {if(value.Code==CREATE_PROCESS_DEBUG_EVENT){trace.Started=true;trace.Active.Add(value.ProcessId);trace.ProcessHandles[value.ProcessId]=value.Info.Create.Process;trace.Processes++;RecordPolicy(trace,value.ProcessId,value.Info.Create.Process);if(trace.Processes>2048)trace.Overflow=true;if(!trace.Overflow)ScanCygheap(trace,value.Info.Create.Process,value.ProcessId,value.Code,unchecked((UInt64)value.Info.Create.ImageBase.ToInt64()),FileIdentity(value.Info.Create.File));}
...
def derive_native(data, mitigation_policy=0):
    ...
    text = once(text, 'public String AppContainerSid,StdoutBase64,StderrBase64;', 'public String AppContainerSid,StdoutBase64,StderrBase64,ForkMemoryDiagnosticBase64,ForkMemoryPolicyBase64; public Int32 ForkMemoryEvents,ForkMemoryProcesses,ForkMemoryLoads,ForkMemorySpans,ForkMemoryQueryError; public Boolean ForkMemoryOverflow;')
    ...
    text = once(text, 'Cancelled=cancelled,StdoutBase64=Convert.ToBase64String(stdout.ToArray()),StderrBase64=Convert.ToBase64String(stderr.ToArray())', 'Cancelled=cancelled,StdoutBase64=Convert.ToBase64String(stdout.ToArray()),StderrBase64=Convert.ToBase64String(stderr.ToArray()),ForkMemoryDiagnosticBase64=Convert.ToBase64String(Encoding.ASCII.GetBytes(forkMemoryTrace.Text.ToString())),ForkMemoryPolicyBase64=Convert.ToBase64String(Encoding.ASCII.GetBytes(forkMemoryTrace.Policies.ToString())),ForkMemoryEvents=forkMemoryTrace.Events,ForkMemoryProcesses=forkMemoryTrace.Processes,ForkMemoryLoads=forkMemoryTrace.Loads,ForkMemorySpans=forkMemoryTrace.Spans,ForkMemoryQueryError=forkMemoryTrace.QueryError,ForkMemoryOverflow=forkMemoryTrace.Overflow')
...
def main():
    ...
    parser.add_argument('--variant', choices=('baseline','heva-off','bottom-up-off'), default='baseline')
    ...
    policies = {'baseline': 0, 'heva-off': 2 << 20, 'bottom-up-off': 2 << 16}
    derived_native, derived_controller = derive_native(native, policies[args.variant]), derive_controller(controller)
    ...
    manifest = {'schema':1,'accepted':False,'purpose':'fork-memory-diagnostic-only','variant':args.variant,'mitigationPolicy':policies[args.variant],'originalNativeSha256':sha(native),'derivedNativeSha256':sha(derived_native),'originalControllerSha256':sha(controller),'derivedControllerSha256':sha(derived_controller),'deriverSha256':sha(pathlib.Path(__file__).read_bytes())}
