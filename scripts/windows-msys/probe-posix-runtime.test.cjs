...
const { readyRecord, fixtureDigest, SOURCE_SHA, MODES } = require('./probe-posix-runtime.cjs')
...
test('literal Bash wrapper passes hostile-looking paths as data and executes real host fixture', { skip: process.platform !== 'linux' }, () => {
  ...
  try {
    ...
    const result = cp.spawnSync('/bin/bash', ['--noprofile', '--norc', '-c', 'output=$1; shift; exec "$@" > "$output"', '--', output, executable, 'pipe-fork', scratch], { encoding: 'utf8', timeout: 25000 })
    ...
    assert.equal(fs.readFileSync(output, 'utf8'), '{"status":"passed","mode":"pipe-fork","supported":true}\n')
