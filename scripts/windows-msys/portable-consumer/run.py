...
def current_environment(env):
    for key,value in {'GITHUB_ACTIONS':'true','RUNNER_OS':'Windows','RUNNER_ARCH':'ARM64','GITHUB_JOB':'platform-primitives','GITHUB_REF':'refs/heads/'+t.BRANCH}.items():t.require(env.get(key)==value,'current ARM workflow identity required')
    ...
    return current,int(env['GITHUB_REPOSITORY_ID'])

def helper_authority(repo,build,system_root):
    record=json.loads(t.physical_read(build/'build.json',1024*1024))
    t.exact(record,['schema','status','identity','source','files'])
    t.require(record['schema']==1 and record['status']=='compiled-native-identity-only' and record['identity']=='bundle-lease-helper-v1:arm64','local ARM helper builder identity')
...
def main(args):
    ...
    current,repository_id=current_environment(os.environ)
    ...
    helper=helper_authority(repo,build,env['SystemRoot'])
    ...
    print('Portable consumer: authenticating exact current-run MSYS artifact',flush=True)
    set_stage('msys-artifact-poll')
    expected=t.resolve_same_run(current,repository_id,os.environ.get('GITHUB_TOKEN'),deadline_seconds=args.artifact_timeout)
    set_stage('msys-artifact-download')
    msys_zip,msys_files,msys_record=t.fetch(expected,os.environ.get('GITHUB_TOKEN'),current)
    ...
    pin=json.loads(t.physical_read(ROOT/'node-arm64-ci20.json',65536))
    print('Portable consumer: authenticating explicitly pinned CI20 ARM64 Node component',flush=True)
    ...
    probe={'candidate':{'authority':context['expected'],'manifestSha256':context['transport']['manifestSha256'],'receiptSha256':receipt['receiptSha256']},
           'node':{'architecture':'arm64','executableSha256':pin['nodeSha256'],'provenanceSha256':pin['provenanceSha256'],'nativeProofSha256':t.digest(t.physical_read(output/'node-native.stdout.txt',256*1024))},
    ...
    print('Portable consumer: executing fresh native ARM64 Node with x64 Bash/MSYS',flush=True)
    set_stage('portable-probe')
    run_process([str(node),str(repo/'scripts/windows-msys/portable-probe/run.cjs'),str(repo),str(imported),str(control/'authority.json'),str(output/'native-probe')],output,'portable-probe',env,1800,emit=True)
...
if __name__=='__main__':
    parser=argparse.ArgumentParser();parser.add_argument('--repo',required=True);parser.add_argument('--output',required=True);parser.add_argument('--controller',required=True);parser.add_argument('--controller-sha256',required=True);parser.add_argument('--helper-directory',required=True);parser.add_argument('--artifact-timeout',type=int,default=2400)
