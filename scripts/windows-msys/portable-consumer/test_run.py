...
class OrchestrationContracts(unittest.TestCase):
 ...
 def test_current_workflow_requires_exact_native_arm_role(self):
  ...
  for key,value in [('RUNNER_ARCH','X64'),('GITHUB_REF','refs/heads/main'),('GITHUB_JOB','other'),('GITHUB_EVENT_NAME','pull_request'),('GITHUB_RUN_ID','123/../'),('GITHUB_REPOSITORY','other/repo'),('GITHUB_SHA','latest')]:
 ...
 def test_local_helper_requires_exact_reviewed_source_and_file_hashes(self):
  with tempfile.TemporaryDirectory() as tmp:
   ...
   value=r.helper_authority(repo,build,'C:\\Windows');self.assertEqual(value['executableSha256'],files[0]['sha256'])
   for change in ['source','exe','arch','extra']:
    ...
    with self.assertRaises(ValueError):r.helper_authority(repo,build,'C:\\Windows')
