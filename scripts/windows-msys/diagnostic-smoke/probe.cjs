...
function runtimeKey(tupleIdentity) {
  const hash = crypto.createHash('sha256')
  for (const name of ['windows-appcontainer.js', 'windows-appcontainer.ps1', 'windows-appcontainer-native.cs', 'windows-appcontainer-command.js', 'windows-appcontainer-probe.js', 'windows-appcontainer-resources.js', 'windows-appcontainer-resources.ps1', 'windows-appcontainer-resources-native.cs', 'windows-helper-deployment.js', 'windows-filesystem.js', 'windows-filesystem.ps1']) hash.update(name).update(fs.readFileSync(name === 'windows-appcontainer-command.js' ? path.join(__dirname,'command.cjs') : name === 'windows-appcontainer-probe.js' ? __filename : path.join(__dirname,'../../../agents/codex/workflow',name)))
  hash.update(tupleIdentity)
...
async function probeWindowsAppContainer(runWindowsAppContainerCommand, tupleIdentity) {
