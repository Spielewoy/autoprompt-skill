...
def derive():
 ...
 for name in ['command','probe']:
  ...
  if name=='command':
   ...
   text=once(text,"phase='acl-write';const acl=cp.spawn(process.env.ComSpec,['/d','/q','/c','icacls ..\\\\\\\\target /grant *'+process.env.AUTOPROMPT_APP_CONTAINER_SID+':F /q > acl-result.txt 2>&1'],{cwd:f.scratch,stdio:'inherit'});const aclExit=await new Promise((ok,no)=>{acl.once('error',no);acl.once('exit',ok)});","phase='acl-write';const aclOutput=fs.openSync(path.join(f.scratch,'acl-result.txt'),'wx');let aclExit;try{const acl=cp.spawn(path.join(process.env.SystemRoot,'System32','icacls.exe'),[f.target,'/grant','*'+process.env.AUTOPROMPT_APP_CONTAINER_SID+':F','/q'],{cwd:f.scratch,stdio:[0,aclOutput,aclOutput]});aclExit=await new Promise((ok,no)=>{acl.once('error',no);acl.once('exit',ok)})}finally{fs.closeSync(aclOutput)};")
