...
struct Native {
  ...
  CreateEvent event;CreateMutant mutant;CreateSemaphore semaphore;CreateSection section;
  Open open_event,open_mutant,open_semaphore,open_section;Query query;
...
void create_hold(const std::wstring &prefix,const std::wstring &sid,const std::wstring &ready,const std::wstring &release){
  auto id=identity();need(id.app&&id.package==sid,"creator-exact-appcontainer");check_prefix(prefix);Native native;std::vector<HANDLE> handles;
  ...
  for(auto &handle:handles){HANDLE owned=handle;handle=NULL;win(CloseHandle(owned)!=FALSE,"close-owned-object");}handles.clear();puts("creator:16:descriptor-and-peer-effects-passed");
  }catch(...){for(auto handle:handles)if(handle)CloseHandle(handle);throw;}
}
