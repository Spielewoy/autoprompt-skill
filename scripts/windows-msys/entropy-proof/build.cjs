...
function main(args) {
  ...
  const sourceFile = path.join(__dirname, 'entropy-proof.cc'), source = bound(sourceFile, 1048576)
  ...
  const copiedSource = path.join(output, 'entropy-proof.cc'), object = path.join(output, 'entropy-proof.obj'), executable = path.join(output, 'entropy-proof.exe')
  ...
  const host = run('host', executable, [], 15000); assert.equal(host.stderr, '')
  const hostProof = parseProof(host.stdout, null)
