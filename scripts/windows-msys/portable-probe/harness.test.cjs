...
test('portable POSIX factoring preserves the original execution, cancellation and positive drain body',()=>{
 ...
 expected=expected.replace("fs.writeFileSync(path.join(runtime, 'posix-proof.exe'), fixture, { flag: 'wx', mode: 0o500 })","fs.writeFileSync(path.join(runtime, 'posix-proof.exe'), fixtureRelocation.bytes, { flag: 'wx', mode: 0o500 })")
 expected=expected.replace("fixtureSha256: fixtureSha, runtime:","fixtureSha256: fixtureSha, fixtureTransformation: fixtureRelocation.receipt, fixtureTransformationRecipeSha256: fixtureRelocationRecipeSha256, runtime:")
 expected=expected.replace("assert.equal(sha(readBounded(path.join(runtime, 'posix-proof.exe'))), fixtureSha)","assert.equal(sha(readBounded(path.join(runtime, 'posix-proof.exe'))), fixtureRelocation.receipt.derivedSha256)")
